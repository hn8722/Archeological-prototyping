const MODEL_DESCRIPTIONS_V1: Record<string, string> = {
  制度:
    "ある価値観を持った人々が日常的に行う習慣をより円滑に行うために作られる制度や、日常の空間とユーザー体験を構成するビジネスを行う関係者(ビジネスエコシステム)がビジネスをより円滑に行うために作られる制度",
  日常の空間とユーザー体験:
    "技術や資源を動員して開発された製品・サービスによって構成される物理的空間であり、その空間で製品・サービスに対してある価値観のもとで意味付けを行い、それらを使用するユーザーの体験",
  前衛的社会問題:
    "技術や資源のパラダイムによって引き起こされる社会問題や日常生活が営まれる空間やそこでのユーザーの体験に対してアート(社会批評)を介して顕在化される社会問題.この問題は誰もが認識しているのではなく、ある一部の先進的/マイノリティの人々のみが認識するもの",
  社会の目標:
    "前衛的社会問題に取り組み先進的なコミュニティによって社会に認識される社会問題やメディアを介して暴露される制度で拘束された社会問題.社会において解決すべき対象として顕在化される",
  技術や資源:
    "日常生活のルーティンを円滑に機能させるために作られた制度のうち、標準化されて過去から制約を受ける技術や資源であり、社会問題を解決すべく組織化された組織(営利・非営利法人、法人格を持たない集団も含み、新規・既存を問わない)が持つ技術や資源",
  ペルソナ:
    "文化芸術振興を通して広められる前衛的社会問題や日常のコミュニケーションによって広められる制度で対応できない社会問題に共感する人々のありたい姿",
  ビジネスエコシステム:
    "日常の空間やユーザー体験を維持するために、それを構成する製品・サービスに関わる関係者が形成するネットワーク",
  アート:
    "人々が気付かない問題を、主観的/内発的な視点で見る人の信念.日常の空間とユーザー体験に違和感を持ち、問題を提示する役割を持つ",
  メディア:
    "現代の制度的欠陥を顕在化させるメディア.マスメディアやネットメディア等の主要なメディアに加え、情報発信を行う個人も含まれる",
  コミュニティ化: "前衛的な問題を認識する人々が集まってできるコミュニティ.公式か非公式かは問わない",
  組織化:
    "社会問題を解決するために形成される組織.法人格の有無や新旧の組織を問わず、社会で広く認識された新しい社会問題に取り組む全ての組織",
